...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Split Sage images into a dataset "
        "(train/val/test) with good/bad air labels."
    )
    parser.add_argument("--csv", type=Path, default=Path("all_data_with_weathervar.csv"))
    parser.add_argument("--images-dir", type=Path, default=Path("images_v2"))
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output directory. Default: images_v2 itself (train/val/test/good/bad).",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=151.0,
        help="PurpleAir PM2.5 value below this is 'good', at/above is 'bad'.",
    )
    parser.add_argument(
        "--imgsz",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Balance good/bad per split by removing surplus majority-class images.",
    )
    parser.add_argument("--seed", type=int, default=42)
    args = parser.parse_args()

    if args.output is None:
        args.output = args.images_dir
    ...
    for (date, label), filenames in sorted(groups.items()):
        ...
        train_end, val_end = split_indices(len(filenames), args.ratios)
        assignments = {
