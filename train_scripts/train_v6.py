#!/usr/bin/env python3
"""
SageAir v6 — Multimodal ResNet50 + meteorology classifier.

Same split, data, and config as v5 but uses ResNet50 (ImageNet pretrained)
instead of CLIP ViT-B/32. Fine-tunes layer4 + head.

Label from purple_air_pm25 >= 151 (bad) vs < 151 (good).
Meteo data matched from all_data_with_weathervar.csv by filename.
"""
import os
import json
import time
...
SAVE_DIR = Path("/home/veday28/SageAir/models/v6_resnet50_pa151_200ep")
...
NUM_EPOCHS = 200
HEAD_LR = 5e-4
BACKBONE_LR = 1e-5   # ResNet layer4 fine-tune (low LR)
WEIGHT_DECAY = 1e-4
DROPOUT = 0.3
LABEL_SMOOTHING = 0.1
...
# ── Model: ResNet50 (fine-tune layer4) + meteo MLP ──────────────────────
class AirQualityModel(nn.Module):
    def __init__(self, meteo_dim=3, dropout=0.3):
        ...
        # Remove the final FC — we get 2048-d features
...
# Two param groups: ResNet layer4 (low LR) + head/proj/meteo (higher LR)
# Use id() for identity checks (same fix as v5)
...
print(f"\n=== Training (v6: ResNet50 layer4 + haze aug + meteo, PA threshold 151, 200 epochs) ===")
...
# ── Save results ────────────────────────────────────────────────────────
results = {
    "version": "v6_resnet50_pa151_200ep",
    ...
    "backbone": "resnet50_imagenet_layer4_finetuned",
    ...
    "split": "70/20/10 stratified, balanced good/bad",
    "history": history,
}
