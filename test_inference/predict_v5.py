#!/usr/bin/env python3
"""
SageAir v5 inference — predict air quality from a single image + meteo data.

Uses the v5 CLIP ViT-B/32 model trained on PurpleAir PM2.5 threshold 151.

Usage:
  python3 predict_v5.py --image <path_to_jpg> --temp 25.3 --humidity 60.2 --pressure 992.0
...
# ── Paths ──────────────────────────────────────────────────────────────
MODEL_DIR = Path("/home/veday28/SageAir/models/v5_clip_haze_pa151")
MODEL_PATH = MODEL_DIR / "best_model.pt"
SCALER_PATH = MODEL_DIR / "meteo_scaler.pkl"

# ── Model definition (must match train_v5.py) ─────────────────────────
class AirQualityModel(nn.Module):
    ...
    def forward(self, img, met):
        feats = self.clip_visual(img).float()
...
def main():
    parser = argparse.ArgumentParser(
        description="SageAir v5 — Predict air quality from image + meteorology (CLIP ViT-B/32, PA threshold 151)"
    )
    ...
    model.eval()
    print(f"Loaded v5 CLIP model from epoch {checkpoint['epoch']} "
    ...
    # Output
    print(f"\n{'='*50}")
    print(f"  Model:         v5 CLIP ViT-B/32 (PA threshold 151)")
