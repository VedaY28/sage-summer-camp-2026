#!/usr/bin/env python3
"""
SageAir v5 test inference — runs the v5 CLIP model on the test set,
saves predictions CSV, metrics CSV, confusion matrix PNG, confidence
histogram PNG, and sample predictions grid PNG.

Usage:
  python3 v5_test_inference.py
  python3 v5_test_inference.py --sample-count 25
"""
...
def plot_confidence_histogram(output_path, predictions):
    ...
    ax.set_title("SageAir v5 — Prediction Confidence Distribution")
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_path, dpi=200)
    plt.close(fig)
...
# ── Main ────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="SageAir v5 test inference + visualizations")
